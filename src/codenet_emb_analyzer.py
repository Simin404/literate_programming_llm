...
logging.info(f'Log saved at: {log_filename}')
...
class EmbeddingAnalyzerNet:
    ...
    def preprocess_desc(self, train_df, test_df):
        if self.model_name == 'embedding_ada':
            ...
            self.indices_train = [i for j,i in enumerate(self.indices_train) if j not in removed_train]
            ...
            self.indices_test = [i for j,i in enumerate(self.indices_test) if j not in removed_test]
    ...
    def analyze(self, train_df, test_df):
        ...
        logging.info(f'train_df: {train_df.shape}; test_df: {test_df.shape}; self.train_emb: {self.train_emb.shape}; length of index: {len(self.indices_train)}')
        train_df = train_df.iloc[self.indices_train].reset_index(drop=True)
        test_df = test_df.iloc[self.indices_test].reset_index(drop=True)
        ...
        logging.info(f'After selection: train_df: {train_df.shape}; test_df: {test_df.shape}; self.train_emb: {self.train_emb.shape}')
    ...
    def map_label(train_label, test_label, desc_label=None):
        le = LabelEncoder()
        # Fit on the union of train + test (and optional desc)
        all_labels = list(train_label) + list(test_label)
        if desc_label is not None:
            all_labels += list(desc_label)
        le.fit(all_labels)

        # Transform
        train_y = torch.tensor(le.transform(train_label), dtype=torch.long)
        test_y = torch.tensor(le.transform(test_label), dtype=torch.long)

        desc_y = None
        if desc_label is not None:
            desc_y = torch.tensor(le.transform(desc_label), dtype=torch.long)
